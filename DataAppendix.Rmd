...
nhis_data2 <- nhis_data %>%
  distinct(NHISPID, .keep_all = TRUE)

nhis_data2 <- nhis_data2 %>% 
  mutate(FOOD_SECURE_NUM=if_else(FOOD_SECURITY=="Secure", 0, 1)) %>%
  group_by(REGION) %>%
  mutate(binned.y = mean(FOOD_SECURE_NUM))

ggplot(nhis_data2, aes(x = REGION, y = logit(binned.y))) + 
  geom_point()
#linearity doesnt really apply because the x is categorical (no specific order to regions). we would not expect it to be linear becauexpse the regions do not have a linear pattern
...
Model: $FoodSecurity = 0.849 - 0.009NorthCentral/Midwest + 0.00437South + 0.073West$
