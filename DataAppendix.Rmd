...
nhis_data2 <- nhis_data2 %>%
  mutate(NUMPREC2 = NUMPREC^3)

m3<-glm(FOOD_SECURITY ~ NUMPREC2 + REGION + AGE + NCHILD + FAMOLDNO + HEALTH_ISSUES + RACE + EDUCATION + FAM_INCOME, data = nhis_data2, family=binomial)
summary(m3)
summary(m2)

ggplot(nhis_data2, aes(x = NUMPREC2, y = logit(binned.y))) + 
  geom_point()
```
